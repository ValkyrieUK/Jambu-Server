...
class Friendship < ActiveRecord::Base
  ...
  def notification
    ...
    friend.device_tokens.each do |e|
      if e.os == 'iOS'
        APNS.send_notification(e.token, "#{user.full_name} is now following you!",:alert => 'Hello iPhone!', :badge => 1, :sound => 'default', :other => {:p => 'esoon', :pid => e.id}) unless e.token == 'NONE' || e.token.nil?
      else
