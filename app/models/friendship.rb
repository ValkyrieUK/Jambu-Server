...
class Friendship < ActiveRecord::Base
  ...
  def notification
    ...
    friend.device_tokens.each do |e|
      if e.os == 'iOS'
        APNS.send_notification(e.token, :alert => "#{user.full_name} is now following you!", :badge => 1, :sound => 'default', :other => {:p => 'foll', :pid => user.uid}) unless e.token == 'NONE' || e.token.nil?
      else
